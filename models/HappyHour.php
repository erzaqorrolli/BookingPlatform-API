...
class HappyHour
{
    ...
    /**
     * Kthen happy hours aktive për një ditë të javës (pa kontroll ore).
     */
    public static function activeForDay(int $companyId, int $dayOfWeek): array
    {
        $db = Database::pdo();
        $stmt = $db->prepare("
            SELECT * FROM happy_hours
            WHERE company_id = ? AND day_of_week = ? AND active = 1
    ...
    /**
     * Kthen happy hour aktiv për një datë + orë specifike.
     */
    ...
    public static function create(int $companyId, array $data): self
    {
        ...
        $stmt = $db->prepare("
            INSERT INTO happy_hours
                (company_id, name, day_of_week, start_time, end_time, discount_percent, active)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([
            $companyId,
    ...
    public function update(array $data): bool
    {
        ...
        $values[] = $this->id;
        $sql = "UPDATE happy_hours SET " . implode(', ', $fields) . " WHERE id = ?";
