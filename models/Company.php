...
class Company
{
    ...
    public static function createWithOwner(
        ...
        try {
            ...
            for ($d = 1; $d <= 6; $d++) {
                $stmt = $db->prepare("
                    INSERT INTO working_hours (company_id, day_of_week, open_time, close_time)
                    VALUES (?, ?, ?, ?)
                ");
    ...
    public static function forUser(int $userId): array
    {
        $db = Database::pdo();
        $stmt = $db->prepare("
            SELECT c.id, c.name, c.slug, c.email, c.phone, c.address,
                   c.logo_url, c.timezone, c.created_at, r.name AS role
